...
PRINTER_WIDTH = 384  # Standard 58mm thermal width (384 dots)

# Target Advertised and GATT Service UUIDs
...
def crc8(data: bytes) -> int:
    """CRC8 lookup table check required by 0x5178 protocol."""
    crc = 0
    for byte in data:
        crc ^= byte
        for _ in range(8):
...
def make_command(cmd: int, payload: bytes = b'') -> bytes:
    """Constructs a 0x5178 protocol packet frame."""
    length = len(payload)
    len_lo = length & 0xFF
...
class TinyPrinterApp:
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(root, width=384, height=300, bg="#f0f0f0", relief="sunken", bd=1)
    ...
    def show_preview(self, path):
        img = Image.open(path)
        img.thumbnail((380, 290))
        self.preview_image = ImageTk.PhotoImage(img)
        self.canvas.delete("all")
        self.canvas.create_image(192, 150, image=self.preview_image)

    def build_print_job(self, image_path, target_width=PRINTER_WIDTH) -> bytes:
        """Converts image to dithered bitmap and wraps it in 0x5178 protocol commands."""
        ...
        job_data.extend(make_command(0xA1, b'\x01'))          # Set quality/speed
        job_data.extend(make_command(0xBE, b'\x00\x00'))      # Initialize session
        job_data.extend(make_command(0xAF, b'\x00\x40'))      # Energy / print density (0x40 = standard)
            ...
            for x in range(0, target_width, 8):
                ...
                for bit in range(8):
                    if x + bit < target_width:
                        ...
                        if pixel == 0:  # 0 is black (dot on)
                            byte_val |= (1 << (7 - bit))
                line_bytes.append(byte_val)

            # Send image line payload (CMD 0xA2 = Print Raster Line)
            job_data.extend(make_command(0xA2, bytes(line_bytes)))

        # Paper feed (CMD 0xA1 with line count) & Finish session
        job_data.extend(make_command(0xA1, b'\x50'))          # Feed 80 dots after printing
        job_data.extend(make_command(0xA3, b'\x01'))          # Print finish sequence
    ...
    async def print_image_async(self):
        ...
        for device, adv_data in devices_and_adv.values():
            ...
            if uuid_match or name_match:
                print(f"Printer Found: {device.name} [{device.address}]")
                printer_device = device
                break

        if not printer_device:
            messagebox.showerror("Error", "Printer not found!\nCheck that printer is ON and disconnected from mobile app.")
        ...
        try:
            ...
            async with BleakClient(printer_device.address) as client:
                ...
                # Send binary payload in 128-byte BLE chunks
                chunk_size = 128
