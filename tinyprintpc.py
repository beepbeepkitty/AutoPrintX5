...
# Pre-calculated bit reversal table (reverses bits in a byte for LSB/MSB fix)
...
class TinyPrinterApp:
    ...
    def build_print_job(self, image_path, target_width=PRINTER_WIDTH) -> bytes:
        """Converts image to dithered 1-bit raw bytes formatted for 0x5178 raster frames."""
        ...
        # Raw bytes from PIL: 1 bit per pixel (0 = black, 1 = white)
        raw_bytes = img_bw.tobytes()
        bytes_per_line = target_width // 8  # 48 bytes per line
        ...
        job_data.extend(make_command(0xAF, b'\x00\x40'))      # Heat Energy / Density
        ...
        for y in range(target_height):
            ...
            # 1. Invert bits (PIL 1=white, thermal paper needs 1=black dot)
            # 2. Reverse bit order per byte for X5 printer controller
            processed_line = bytes(BIT_REVERSE_TABLE[~b & 0xFF] for b in line)

            # Send line raster command (0xA2)
            job_data.extend(make_command(0xA2, processed_line))

        # Paper feed & end session
        job_data.extend(make_command(0xA1, b'\x50'))          # Feed paper
    ...
    async def print_image_async(self):
        ...
        try:
            ...
            async with BleakClient(printer_device.address) as client:
                ...
                # Write in 128-byte chunks with 20ms delay to prevent buffer drop
                chunk_size = 128
